...
function setupPicker() {
  const max = allQuestions.length; // ← use FULL length, no artificial cap
  qSlider.min = 1;
  qSlider.max = max;
  ...
  // Show max 20 ticks to avoid overflow
  ...
  // Always add last number if not there
  ...
  document.querySelectorAll(".qp-btn").forEach(btn => {
    ...
    if (raw === "all") {
      btn.dataset.val = max; // ← store ACTUAL max
...
document.querySelectorAll(".qp-btn").forEach(btn => {
  btn.onclick = () => {
    ...
    const val = parseInt(btn.dataset.val); // already set to actual max for "all"
    qSlider.value = val;
...
document.getElementById("startSession").onclick = () => {
  ...
  questions = shuffle([...allQuestions]).slice(0, count); // slice by chosen count
...
// ① Try Again → back to question picker (same mode = quiz)
...
// ② Upload new PDF → full reload (simplest & safest)
document.getElementById("endUpload").onclick = () => {
  location.reload();
};

// ③ Try Flashcards → go to picker in flash mode
